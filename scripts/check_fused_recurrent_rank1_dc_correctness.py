...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare fused recurrent rank-1 DC against the naive recurrent reference.",
    )
...
def main() -> None:
    ...
    if args.with_initial_state:
        initial_state = (
            0.05 * torch.randn(shape_h0, device=device, dtype=dtype),
            0.05 * torch.randn(shape_b0, device=device, dtype=dtype),
        )
    ...
    test_out, test_final = fused_recurrent_gated_delta_rule_rank1_dc(
        q=test_inputs[0],
        k=test_inputs[1],
        v=test_inputs[2],
        beta=test_inputs[4],
